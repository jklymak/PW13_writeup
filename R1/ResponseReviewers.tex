...
\begin{reply}
    ...
    \begin{lstlisting}[language=TeX, basicstyle=\small]
        ...
        along-shore wind forcing, cross-shore transport is supplied by Ekman layers.
        However, in many locations there is also evidence of eddies, meanders and
        filaments driving wholesale separation of shelf currents into the deep ocean.
        These have been well-studied off California \cite{strubetal91} where satellite
...
\begin{reply}
    ...
    48.4\textdegree N, which is sufficient for the limited geographic extents
    discussed here.
    \end{lstlisting}
...
\begin{reply}
    We agree that there is upwelling, and that this upwelling tends to push water onto the shelf.  However, its not clear why that would be stronger here than further upstream (poleward) where the water has had time to partially mix.  Further, coastal upwelling  brings water up at shelf-break depth, whereas this is happening at mid-depth.  If anything, you'd expect the secondary circulation associated with upwelling to push mid-depth water offshore.

    To help clarify this, we have added to the end of the second paragraph of section 3.3:
    \begin{lstlisting}
        The high-spice water comes onto the shelf through much of the water column, so is
        not just being carried in a near-bottom Ekman layer by wind driven upwelling.
    \end{lstlisting}
\end{reply}
...
Its hard to say more, but we suspect that the model has a more energetic mesoscale eddy field than observed, and these eddies generate substantial cross-shore motions.  It would be a great project to try and test why the regional simulation is so much more advective, but beyond our scope here.
\end{reply}
...
\shortreply[]{Thanks, corrected}


\shortpoint{Fig 12: label Barclay Canyon}
\shortreply[]{We tried this, but its pretty hard to do without obscuring data. Barkley Canyon is labelled in Figure 1.  }
...
\begin{reply}
Indeed, keeping this all straight is challenging, and thanks for your suggestions.  Figure 1 now has three panels, and we changed the layering.  This is probably worth the extra space, and we hope that it helps with legibility.  We added the La Perouse Buoy to Fig 1c.  Hopefully this helps, though with such a complicated study region and diverse data sets, it will remain challenging.
\end{reply}
...
\begin{reply}
    Thanks cleaned this up in a few other places.  We gree that using ``this'' at the beginning of a sentence should be clear and unambiguous.  On the other hand, repeating a phrase from the previous sentence (or worse rephrasing it) can be harder to parse.
\end{reply}
...
\begin{reply}
    ...
    for the banks and canyons, we capitalized if it was used as a proper noun,
    and left lower case if referring to a generic, e.g. ``the canyon''.
    ...
    distances.  (The lead author will admit thinking he was illiterate between
    moves from Canada, to the US, and back again in his spelling of
    ``modeling/modelling'')
\end{reply}
...
\begin{reply}
    Thanks. We changed these lines to:
    \begin{lstlisting}
...
\shortreply[]{Thanks, this is fixed.}
...
\begin{reply}
    Indeed, we have now split it.  The second sentence is still rather long,
    but is a list, so should be easy enough to parse.
\end{reply}
...
\begin{reply}
    They are generally about 2-4\%.  We are using oxygen very qualitatively in this paper, so we have removed the discussion of the recalibration that is routinely done.
\end{reply}
...
\begin{reply}
    We kept this the same just for ease of skimming by readers.  The lack of an ADCP was very disappointing, and its something we expect readers will be looking for.  Hence the standalone paragraph to make it easy to find.
\end{reply}
...
\begin{reply}
    ...
    \begin{lstlisting}
        ...
        summer conditions on the Southern Vancouver Island Shelf and are presented
...
\begin{point}
    ...
    165: the coordinate system is not described in the main text, just in the figure caption. Probably worth adding to section 2? Also helpful to reference the $26.4\ \mathgrm{kg\,m^{-3}}$ isopycnal as magenta in the text here, especially if it is marked in magenta the rest of the paper.
\end{point}

\begin{reply}
    Thanks. There is a new second paragraph in the Introduction explaining the co-ordinate system.

    \begin{lstlisting}
        We use an along/across-shelf co-ordinate system with an origin at
        48.4\textdegree N and 125.6\textdegree W, with the across-shelf $x$ oriented
        39 degrees north of East (\fref{fig:LocMapBoth}b, grey/white alternating
        lines).  The projection is Cartesian with a central latitude at
        48.4\textdegree N, which is sufficient for the limited geographic extents
        discussed here.
    \end{lstlisting}

    The $26.4$ isopyncal is referenced in the text.

    \begin{lstlisting}
        Note that the water along the $26.4\ \mathrm{kg\,m^{-3}}$ isopycnal (\fref{fig:LaPerouse2013Ctd}, magenta contours) becomes cooler...
    \end{lstlisting}
...
\begin{reply}
  Thanks, great suggestion.  We now identify the water masses by values of spice anomaly.
\end{reply}
...
\begin{reply}
    The other reviewer had a similar suggestion - we have made a PDF of $gamma$ along 26.4, and identified the peaks.  Readers may be more comfortable with 1-D PDFs anyhow, so this seems a good way to clarify how we are definiting the three water masses.
\end{reply}
...
\begin{reply}
    ...
    \begin{lstlisting}
        ...
        casts at a given depth are given a Gaussian weight $w = \exp^{-(r/r_0)^2}$,
...
\begin{reply}
    That is unfortunate because it is one of the main points of the paper!    The new version of Fig 12 has all the data, which we probably should have used in the original version because hopefully it makes the tongue more clear.  We also now refer to this panel explicitly in the text.  We could annotate it, but the plot is already relatively dense.
\end{reply}
...
\begin{reply}
    Thanks, done. We also changed the km markings to the turn locations and added ticks on panel d corresponding to the tick locations.
\end{reply}
...
\begin{reply}
    Fixed; it was the MVP data, though the previous version was a subsample of the full dataset.
\end{reply}
...
\begin{reply}
    Yes, but perhaps better shown in Fig 3e, k, which are now explicitly referenced here.
\end{reply}
...
\begin{reply}
    This was just a typical weak coastal current scaling.  We now cite Thomson and Kassovski (2015), and point out that this current could be substantially stronger.
\end{reply}
